...
export default function Game({
    ...
    let cards = Object.keys(pokemonDetailsSet).map((key) => {
        return (
            <div className="cards" key={key} onClick={memorize.bind(null, key)}>
                <img
                    src={pokemonDetailsSet[key]["sprites"]["front_default"]}
                    alt={pokemonDetailsSet[key]["name"]}
                ></img>
                <div className="name">
                    {pokemonDetailsSet[key]["name"].toString()}
                </div>
            </div>
        );
    });
